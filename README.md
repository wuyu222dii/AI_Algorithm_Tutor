...
生产构建后运行 `pnpm budget:coach`，会检查学习中心、题库、进度和复习页的独有 JavaScript（gzip 不超过 200 KiB）、实际 73 题 `ProblemSummary` RSC 序列化 payload（gzip 不超过 150 KiB）及浏览器 Runner Worker 资产（不超过 6 MiB）。这些非代码执行页面若意外引入 Monaco、QuickJS、TypeScript 编译器或 Pyodide，检查会直接失败。`pnpm check` 和 GitHub Production build 均包含该门禁。

浏览器通过 Next.js Web Vitals hook 采集 LCP、INP 和 CLS，并将数值、评级和预算状态发送到 Sentry Metrics；上限分别为 2.5 秒、200 毫秒和 0.1。上报不包含 URL、用户、DOM 节点、题面或代码，`NEXT_PUBLIC_WEB_VITALS_SAMPLE_RATE` 控制会话采样率，生产初始建议设为 `0.10`。
...
迁移 `0023` 启用访客数据耐久认领、匿名漏斗和用户时区口径。Staging 先同时设置
`DURABLE_GUEST_CLAIM_ENABLED=true` 与
`NEXT_PUBLIC_DURABLE_GUEST_CLAIM_ENABLED=true`；服务端收到幂等 ACK 前不会清除访客命名空间。
`ANONYMOUS_METRICS_ENABLED=true` 后，浏览器只批量上传白名单事件 ID、名称、时间和可选题目 slug；服务端使用
`ANONYMOUS_METRICS_HMAC_SECRET`（未配置时回退 `AUTH_SECRET`）生成匿名 subject，不保存 Cookie、IP、代码或题面。
两个功能在 Staging 验证后再以相同配置开启到 Production。
...
`pnpm test:coverage` 对公测关键协议执行强制覆盖率门禁：lines/functions/statements 不低于 80%，branches 不低于 75%。统计范围包括同步、测评、AI 路由、模型故障转移、限流、学习证据、复习评分、匿名事件、访客认领和 readiness；大型 UI 交互与数据库持久化路径分别由 Playwright 和 PostgreSQL 集成测试承担，避免用低价值模板文件稀释关键域覆盖率。
...
- GitHub Environment Secret：`VERCEL_TOKEN`、`AI_RELAY_CANARY_TOKEN`
- GitHub Environment Variable：`VERCEL_ORG_ID`、`VERCEL_PROJECT_ID`

先从 Actions 手动选择 `staging`，完成一次真实 Google 测试账号验收；生产发布只允许从 `main` 发起。release job 成功后会发布带 SBOM 和 provenance 的不可变 GHCR 镜像，再使用 Vercel CLI 构建并部署同一提交，最后对实际部署 URL 执行 live、ready 和 AI relay canary。Vercel Preview 与 Production 的运行环境变量需要分别配置，GitHub Environment 变量不会自动注入 Vercel。
...
在 `staging` 和 `production` GitHub Environment 中配置 Variable `DEPLOYMENT_BASE_URL`（只填写部署后的 HTTPS Origin）、可选 Variable `DEPLOYMENT_HEALTH_TIMEOUT_MS`，以及 Secret `AI_RELAY_CANARY_TOKEN`。该 Secret 必须与对应部署运行环境中的 `AI_RELAY_CANARY_TOKEN` 完全一致，不能使用中转站 API Key 代替。若 Vercel Preview 或 Production 开启 Deployment Protection，还需在对应 GitHub Environment 配置 Secret `VERCEL_AUTOMATION_BYPASS_SECRET`；未开启保护时可留空。

`AlgoCoach Deployment Canary` Workflow 在 UTC 每小时第 17、47 分钟自动检查 production，也可手动选择 staging 或 production。尚未配置 production `DEPLOYMENT_BASE_URL` 时，定时任务会记录 Warning 并正常跳过；手动执行仍会严格校验缺失配置。配置完成后，它严格按 live → ready → AI canary 顺序执行，前一步失败时不会继续发送 canary 凭据；因此可以同时覆盖部署进程、环境配置、数据库与迁移、Redis、认证和真实中转站主备模型。也可以用同一组环境变量运行 `pnpm health:deployment` 做部署后人工验收。探针输出只包含 Origin、状态、延迟和错误分类，不包含 Token 或响应正文。

Vercel Git 自动部署已在 `vercel.json` 禁用，避免绕过质量检查、迁移和 AI Gate。首次合入该配置后需从 GitHub Actions 手动运行一次 staging release，让 Vercel 项目采用新配置；生产发布会先创建不绑定域名的候选部署，通过 readiness 与 AI canary 后才执行 promote。

生产环境必须配置 HTTPS REST 形式的 `REDIS_URL` 与 `REDIS_TOKEN`。Vercel/Upstash 自动注入的 `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` 或 `KV_REST_API_URL` / `KV_REST_API_TOKEN` 完整变量对也可直接使用。readiness 会执行无副作用的 `PING` 与 `EVAL return 1`，确保凭据具备限流和并发租约所需的 Lua 执行权限；远程明文 HTTP 会被拒绝，`REDIS_ALLOW_INSECURE_LOCAL=true` 仅供 Docker smoke 连接本机 mock。开发环境未配置 Redis 时继续使用进程内限流，不影响本地启动。AI readiness 只校验模型、Base URL 和凭据配置；真实中转站连通性由受保护 canary 与发布 AI Gate 检查，避免中转站短时故障触发实例重启循环。
