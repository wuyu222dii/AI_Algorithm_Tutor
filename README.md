...
在 `staging` 和 `production` GitHub Environment 中配置 Variable `DEPLOYMENT_BASE_URL`（只填写部署后的 HTTPS Origin）、可选 Variable `DEPLOYMENT_HEALTH_TIMEOUT_MS`，以及 Secret `AI_RELAY_CANARY_TOKEN`。该 Secret 必须与对应部署运行环境中的 `AI_RELAY_CANARY_TOKEN` 完全一致，不能使用中转站 API Key 代替。
