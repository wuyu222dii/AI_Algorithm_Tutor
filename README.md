...
`AlgoCoach Deployment Canary` Workflow 在 UTC 每小时第 17、47 分钟自动检查 production，也可手动选择 staging 或 production。它严格按 live → ready → AI canary 顺序执行，前一步失败时不会继续发送 canary 凭据；因此可以同时覆盖部署进程、环境配置、数据库与迁移、Redis、认证和真实中转站主备模型。也可以用同一组环境变量运行 `pnpm health:deployment` 做部署后人工验收。探针输出只包含 Origin、状态、延迟和错误分类，不包含 Token 或响应正文。
